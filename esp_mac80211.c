...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 30) && LINUX_VERSION_CODE < KERNEL_VERSION(4, 7, 00))
#include <net/regulatory.h>
#elif (LINUX_VERSION_CODE > KERNEL_VERSION(4, 7, 00))
...
static bool beacon_tim_alter(struct sk_buff *beacon)
{
        ...
        while (remain_len > 0)
        {
                ...
                { // tim field
                        ...
                        if (tim_count == 0)
                                *p |= 0x1;
                        if ((*p & 0xfe) == 0 && tim_end >= p + 1)
                        { // we only support 8 sta in this case
                                p++;
                                *p = beacon_tim_save(*p);
                        }
                        return tim_count == 0;
...
{
        struct ieee80211_vif *vif = (struct ieee80211_vif *)tl;
        struct esp_vif *evif = (struct esp_vif *)vif->drv_priv;
...
static int esp_op_ampdu_action(struct ieee80211_hw *hw,
                               ...
                               enum ieee80211_ampdu_mlme_action action,
                               struct ieee80211_sta *sta, u16 tid, u16 *ssn)
#elif (LINUX_VERSION_CODE < KERNEL_VERSION(4, 8, 00))
static int esp_op_ampdu_action(struct ieee80211_hw *hw,
                               struct ieee80211_vif *vif,
                               enum ieee80211_ampdu_mlme_action action,
                               struct ieee80211_sta *sta, u16 tid, u16 *ssn,
...
#else
                ESP_IEEE80211_DBG(ESP_DBG_ERROR, "%s RX START %pM tid %u %u\n", __func__, sta->addr, tid, params->ssn);
                ret = sip_send_ampdu_action(epub, SIP_AMPDU_RX_START, sta->addr, tid, params->ssn, 64);
#endif
                break;
